...
const ClassManagement = ({ token }) => {
  ...
  const handleSaveClass = async (updatedClass) => {
    try {
      ...
      formData.append('className', updatedClass.className);
      formData.append('description', updatedClass.description);
      if (updatedClass.image) formData.append('image', updatedClass.image);

      // Levels als JSON-String hinzufügen
      if (updatedClass.levels && updatedClass.levels.length > 0) {
        formData.append('levels', JSON.stringify(updatedClass.levels));
      }
      else
      {
      ...
      if (modalMode === 'add') {
        const res = await axios.post(`${API_URL}/api/classes`, formData, config);
        setClasses([...classes, res.data]);
      } else if (modalMode === 'edit') {
        const res = await axios.put(`${API_URL}/api/classes/${selectedClass.id}`, formData, config);
        setClasses(classes.map((cls) => (cls.id === selectedClass.id ? res.data : cls))); // Antwort des Servers verwenden
      }
    } catch (error) {
